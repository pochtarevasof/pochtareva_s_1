...
Функция РесурсAPI(ПараметрыОбмена)
...
Функция РесурсЗаявленияНаКомпенсациюОтпуска()
	...
	Возврат "/leave-encashment-requests";
...
Функция РесурсФизическиеЛицаАктивные()
	...
	Возврат РесурсФизическиеЛица() + "/active";

КонецФункции

Функция РесурсФизическиеЛицаНеактивные()

	Возврат РесурсФизическиеЛица() + "/inactive"; 
...
Функция HTTPОтветСервиса(ПараметрыОбмена, РесурсСервиса, ИмяМетода, СтрокаТела = "", ОписаниеФайла = Неопределено, РазрешенныйКодОтвета = "")
	...
	Если ЗначениеЗаполнено(ОписаниеФайла) Тогда
		...
		Если КабинетСотрудника.ИспользоватьРасширениеФайлаВЗаголовке() Тогда
	...
	АдресРесурса = СтрШаблон("/%1%2%3",ПараметрыОбмена.СтруктураАдреса.ПутьНаСервере,РесурсAPI(ПараметрыОбмена),РесурсСервиса);
...
Функция ОписаниеОбъектаИнформацияОПриложении()
...
Функция ДанныеРасчетныхЛистов(Организация, МесяцРасчетныхЛистов, СписокФизическихЛиц, ПовторнаяПубликация)
	...
	РасчетныеЛистыДокументы = Данные.РасчетныеЛистыДокументы;
	...
	Для каждого ЭлементКоллекции Из РасчетныеЛистыДокументы Цикл
		...
		ИнформацияОЗарплате.Вставить(Отбор.ФизическоеЛицо, ОбъектИнформацияОЗарплате);
...
Функция ДанныеРасчетныхЛистовДляПубликации(Организация, Месяц, СписокФизическихЛиц)
	...
	Если ЗначениеЗаполнено(Данные.ДанныеРасчетныхЛистков) Тогда
		...
		Результат.РасчетныеЛистыДокументы = РасчетныеЛистыДокументы(Данные.ДокументРезультат);
		Возврат Результат;
	КонецЕсли;
...
Функция РезультатВыполненияОбмена(ВестиПротокол) Экспорт
	...
	Попытка
		...
		Если Не Константы.СервисКабинетСотрудникаВЛокальнойСети.Получить() Тогда
			Если Не НастройкиСервиса.ТестовыйТарифАктивирован Тогда
				Если Не ОбщегоНазначения.РазделениеВключено() Тогда
					КабинетСотрудникаМенеджерСервиса.АктивироватьТестовыйТариф(ВестиПротокол);
				КонецЕсли;
			ИначеЕсли Не НастройкиСервиса.СообщениеОбАктивацииТестовогоТарифаОтправлено Тогда
				КабинетСотрудникаМенеджерСервиса.ОтправитьСообщениеОбАктивацииТестовогоТарифа(ВестиПротокол);
			КонецЕсли;
			Если Не НастройкиСервиса.АдресПриложенияПоИмениДоступен И ЗначениеЗаполнено(НастройкиСервиса.АдресПриложенияПоИмени) Тогда
		...
		ПроверитьВерсиюСервиса(ПараметрыОбмена);
		...
		ОпубликоватьУдалениеДокументовНаПодпись(ПараметрыОбмена, БылиОшибки);
...
Процедура ОпубликоватьУдалениеДокументовНаПодпись(ПараметрыОбмена, БылиОшибки)
...
Процедура ЗагрузитьНезагруженныеОбъекты(ПараметрыОбмена, БылиОшибки)
	...
	ТипОбъектаРесурс 		= ТипОбъектаРесурсСервиса();
...
Функция ТипОбъектаРесурсСервиса()
	...
	РесурсСтатусов.Вставить(Перечисления.ТипыОбъектовКабинетСотрудника.РезультатСогласования, РесурсРезультатыСогласования());
...
Процедура ЗарегистрироватьНезагруженныйИдентификатор(ИдентификаторОбъекта, ТипОбъекта, ЗарегистрироватьУдаление = Ложь)
...
Функция ИзмененияИзСервиса(ПараметрыОбмена, ВерсияИзменений, Лимит)
	...
	РесурсСервиса = СтрЗаменить(РесурсСервиса, "{limit}", Лимит);
...
Функция ОбработатьИзмененияИзСервиса(ПараметрыОбмена, ПолученныеИзменения, ПовторнаяЗагрузка = Ложь)
	...
	// Обработка сохраненных необработанных результатов согласования.
...
Функция ЗагрузитьРезультатыСогласования(ПараметрыОбмена, ПолученныеИзменения, РезультатОбработки, ВерсияИзменений)
	...
	Для каждого СтрокаТЗ Из ТаблицаРезультатовСогласований Цикл
		...
		Попытка
			...
			ЗагрузитьРезультатСогласования(ИспользуетсяКадровыйЭДО, РезультатСогласования, СтрокаТЗДокументКЭДО, ДокументыКЭДОРасчетныеЛистки, ИдентификаторыДокументовПовторнойПубликации);
			ЗафиксироватьТранзакцию();
...
Процедура ЗагрузитьРезультатСогласования(ИспользуетсяКадровыйЭДО, РезультатСогласования, СтрокаТЗДокументКЭДО, ДокументыКЭДОРасчетныеЛистки, ИдентификаторыДокументовПовторнойПубликации)
...
Функция ВыгрузитьВложенияВСервис(ПараметрыОбмена, ТаблицаДанных, Вложения, ИмяПоляВладельца, НазваниеВложения)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДанных Цикл
		...
		Ответ = HTTPОтветСервиса(ПараметрыОбмена, РесурсСервиса, "POST",, ОписаниеФайла);
...
Функция РезультатВыгрузкиФизическихЛиц(ПараметрыОбмена, Список)
	...
	Если ЗначениеЗаполнено(ДанныеФизическихЛиц.Вложения) Тогда
		...
		БылиОшибки = ВыгрузитьВложенияВСервис(ПараметрыОбмена, ТаблицаДанных, ДанныеФизическихЛиц.Вложения, "ФизическоеЛицо", "Фотография");
	КонецЕсли;
	...
	Иначе
		...
		Для каждого СтрокаТЗ Из УсловноПубликуемыеСотрудники Цикл
			...
			Если Публикуется = Неопределено Тогда
				...
				Ответ = HTTPОтветСервиса(ПараметрыОбмена, РесурсСервиса, "GET",,,404);
				Если Ответ = Неопределено Тогда
...
Функция ОпубликоватьСтатусЗаявки(ПараметрыОбмена, РесурсЗаявкиСервиса, ИдентификаторЗаявки, Статус)
	...
	Ответ = HTTPОтветСервиса(ПараметрыОбмена, РесурсСервиса, "PUT");
...
Функция ЭлектронныйДокументРезультатыСогласования(ПараметрыОбмена, ДанныеЭлектронногоДокумента, ИдентификаторДокумента)
	...
	ОбъектИсходныйДокумент = ОбъектСервисаФайл(ИдентификаторФайла, Оригинал.ИмяФайлаБезРасширения, Оригинал.РасширениеФайла, Оригинал.Размер, ВерсияФайла);
	...
	Для каждого ПредставлениеДокумента Из Представления Цикл
		...
		ОбъектФайл = ОбъектСервисаФайл(ИдентификаторФайла, ПредставлениеДокумента.ИмяФайлаБезРасширения, ПредставлениеДокумента.РасширениеФайла, ПредставлениеДокумента.Размер);
...
Функция ОбъектФайлПоПрисоединенномуФайлу(ПрисоединенныйФайл, ПараметрыОбмена)
	...
	Возврат ОбъектСервисаФайл(ИдентификаторФайла, ДанныеФайла.ИмяФайла, ДанныеФайла.Расширение, ДанныеФайла.Размер, ВерсияФайла);

КонецФункции

Функция ОбъектВходящийФайлПоПрисоединенномуФайлу(ПрисоединенныйФайл, ПараметрыОбмена)
...
Функция РезультатПубликацииРасчетныхЛистков(ДанныеДляПубликации, ПовторнаяПубликация, ВестиПротокол, ПерваяПоловинаМесяца) Экспорт

	Результат = Новый Структура("БылиОшибки", Ложь);
	
	ЗаписатьСобытиеНачалоПубликацииРЛ();
	
	ПараметрыПубликации = НовыеПараметрыПубликацииРЛ();
	ПараметрыПубликации.ПовторнаяПубликация = ПовторнаяПубликация;
	ПараметрыПубликации.ВестиПротокол = ВестиПротокол;
	ПараметрыПубликации.ПерваяПоловинаМесяца = ПерваяПоловинаМесяца;
	...
	Попытка
		...
		Для каждого ЭлементКоллекции Из ДанныеДляПубликации Цикл
			...
			ПараметрыПубликации.Месяц = ЭлементКоллекции.Ключ;
...
Функция НовыеПараметрыПубликацииРЛ()
...
Функция РезультатВыгрузкиРасчетныхЛистков(ПараметрыОбмена, ПараметрыВыгрузки) 
	...
	Данные = ДанныеРасчетныхЛистов(Организация, Месяц, СписокФизическихЛиц, ПараметрыВыгрузки.ПовторнаяПубликация);
	...
	ФизическиеЛицаИменаФайлов = ФизическиеЛицаИменаФайловРЛ(СписокФизическихЛиц, Месяц);
	...
	Для каждого ФизическоеЛицо Из СписокФизическихЛиц Цикл
		...
		Попытка
			...
			Если ПубликоватьДокументыНаПодпись Тогда
				...
				ОбъектФайл = ОбъектСервисаФайл(ИдентификаторФайла, ИмяФайла, РасширениеФайла, ДанныеФайлаРЛ.РазмерФайла, ВерсияФайла);
...
Функция ФизическиеЛицаИменаФайловРЛ(СписокФизическихЛиц, Месяц)
...
Процедура ОбновитьНастройкиФункциональностиСервиса(ПараметрыОбмена = Неопределено) Экспорт
	...
	НовыеНастройки = КабинетСотрудника.ДоступнаяФункциональностьСервиса();
...
Функция ОбъектСервисаНастройкиФункциональности(НастройкиФункциональности)
	...
	ОписаниеОбъекта = ОписаниеОбъектаНастройкиФункциональности();
...
Функция ОписаниеОбъектаНастройкиФункциональности()
...
Функция ЗапросОбновитьНастройкиЗаявокНаОтпуск(ПараметрыОбмена, Настройки)
	...
	ОбъектСервиса = ОбъектСервисаНастройкиЗаявокНаОтпуск(Настройки);
	ЗаписьJSON = Новый ЗаписьJSON;
	ЗаписьJSON.УстановитьСтроку();
	ЗаписатьJSON(ЗаписьJSON, ОбъектСервиса);
	СтрокаТела = ЗаписьJSON.Закрыть();
...
Функция ОбъектСервисаНастройкиЗаявокНаОтпуск(Настройки)
...
Функция ДеактивироватьФизическихЛиц(ФизическиеЛица) Экспорт
	...
	ПараметрыОбмена = ПараметрыОбмена(Ложь);
	Ответ = ОбновитьАктивациюФизическихЛицСервиса(ПараметрыОбмена, РесурсСервиса, ФизическиеЛица);
	Возврат Не Ответ.БылиОшибки;

КонецФункции

Функция ИдентификаторыОпубликованныхФизическихЛиц() Экспорт
...
Процедура АктивироватьФизическихЛицПоРезультатуВыгрузки(ПараметрыОбмена, РезультатВыгрузки)
	...
	РесурсСервиса = РесурсФизическиеЛицаАктивные();
	Ответ = ОбновитьАктивациюФизическихЛицСервиса(ПараметрыОбмена, РесурсСервиса, РезультатВыгрузки.Выгружено);
	РезультатВыгрузки.БылиОшибки = РезультатВыгрузки.БылиОшибки Или Ответ.БылиОшибки;
	Если Ответ.БылиОшибки Тогда
		Если ЗначениеЗаполнено(Ответ.Выгружено) Тогда
			РезультатВыгрузки.Выгружено = Ответ.Выгружено;
		Иначе
...
Функция ОбновитьАктивациюФизическихЛицСервиса(ПараметрыОбмена, Знач РесурсСервиса, Знач СписокФизическихЛиц, РазмерПакета = 100)
	
	Результат = Новый Структура("Выгружено,НеВыгружено,БылиОшибки",Новый Массив,Новый Массив,Ложь);
	
	КоличествоЭлементов = СписокФизическихЛиц.Количество();

	Если КоличествоЭлементов > РазмерПакета Тогда
		ДанныеПакета = Новый Массив;
		Для Сч = 1 По РазмерПакета Цикл
			ДанныеПакета.Добавить(СписокФизическихЛиц[0]);
			СписокФизическихЛиц.Удалить(0);
		КонецЦикла;
		Результат = ОбновитьАктивациюФизическихЛицСервиса(ПараметрыОбмена, РесурсСервиса, СписокФизическихЛиц, РазмерПакета);
	Иначе
		ДанныеПакета = СписокФизическихЛиц;
	КонецЕсли;
	...
	Если Ответ = Неопределено Тогда
		Результат.БылиОшибки = Истина;
	ИначеЕсли Ответ.КодСостояния = 200 Тогда
		Для Каждого ФизическоеЛицо Из ДанныеПакета Цикл
			Результат.Выгружено.Добавить(ФизическоеЛицо);
		КонецЦикла;
	ИначеЕсли Ответ.КодСостояния = 400 Тогда
		...
		Если Результат <> Неопределено Тогда
			Для Каждого ЭлементРезультат Из Результат Цикл
				Результат.Выгружено.Добавить(ДанныеПакета[Число(ЭлементРезультат["position"]) - 1]);
			КонецЦикла;
		КонецЕсли;
		...
		Если Ошибки <> Неопределено Тогда
			Для Каждого ЭлементОшибки Из Ошибки Цикл
				...
				Если ПозицияВКоллекции <> Неопределено Тогда
					НомерПозиции = Число(ПозицияВКоллекции);
					Если НомерПозиции <= КоличествоЭлементов Тогда
						Результат.НеВыгружено.Добавить(ДанныеПакета[НомерПозиции - 1]);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Функция ОбновитьНастройкиСистемыВзаимодействия(ПользовательСВ, ИдентификаторОбсужденияСВ) Экспорт
	...
	ТелоЗапроса.Вставить("botID", 					Строка(ПользовательСВ.Идентификатор));
	ТелоЗапроса.Вставить("botName", 				ПользовательСВ.Имя);
	ТелоЗапроса.Вставить("systemConversationID", 	ИдентификаторОбсужденияСВ);
...
Функция ИнформацияЛокальногоПриложения() Экспорт
	...
	ПараметрыОбмена = ПараметрыОбмена(Ложь);
	ИнформацияПриложения = ЗапросИнформацияОПриложении(ПараметрыОбмена);
	
	Если ЗначениеЗаполнено(ИнформацияПриложения.ВерсияПриложения) Тогда
		РегистрыСведений.НастройкиСервисаКабинетСотрудника.УстановитьВерсиюПриложения(ИнформацияПриложения.ВерсияПриложения);
	КонецЕсли;
	
	Возврат ИнформацияПриложения;
...
Функция УстановитьАдресЛокальногоПриложения(АдресПриложения) Экспорт
	...
	НастройкаПриложения.Вставить("applicationUrl", АдресПриложения);
	ЗаписьJSON = Новый ЗаписьJSON;
	ЗаписьJSON.УстановитьСтроку();
...
Функция УстановитьАдминистратораЛокальногоПриложения(ОписаниеАдминистратора) Экспорт
	...
	Ответ = HTTPОтветСервиса(ПараметрыОбмена, РесурсСервиса, "POST", СтрокаТела);
...
Процедура ПроверитьВерсиюСервиса(ПараметрыОбмена)
...
Функция ЗапросИнформацияОПриложении(ПараметрыОбмена)
	...
	Если Ответ <> Неопределено И Ответ.КодСостояния = 200 Тогда
		...
		Попытка
			ОбъектОтвета = ПрочитатьJSON(ЧтениеJSON, Истина);
			ИнформацияОПриложении = СтруктураИзОбъектаСервиса(ОбъектОтвета, ОписаниеОбъектаИнформацияОПриложении());
		Исключение
			КабинетСотрудника.ЗаписатьОшибкуЧтенияJSON(Ответ.ПолучитьТелоКакСтроку(), ОписаниеОшибки());
		КонецПопытки;
	КонецЕсли;
...
Функция ОпубликоватьДвоичныеДанныеФайла(ПараметрыОбмена, ФайлДвоичныеДанные, Расширение)
