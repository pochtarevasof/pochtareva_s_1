...
Функция ИспользуютсяЭлектронныеДокументы(ПоДаннымИБ = Ложь)
	...
	Если ПоДаннымИБ Тогда
		...
		Если Не Используются Тогда
			...
			Запись.Прочитать();
			Если Запись.Выбран() И Запись.ИспользуетсяКадровыйЭДО Тогда
				Используются = Истина;
			КонецЕсли;
...
Процедура ДобавитьНастройкуПечатнойФормы(ОписанияНастроек, ИдентификаторПечатнойФормы, СодержимоеДокумента, БлокировкаСУсловием = Ложь, НаСписокСотрудников = Ложь) Экспорт
