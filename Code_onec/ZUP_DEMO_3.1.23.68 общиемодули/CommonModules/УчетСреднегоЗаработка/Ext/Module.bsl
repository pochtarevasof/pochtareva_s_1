...
Процедура РаспределитьВТНачисленияПоБазе(МенеджерВременныхТаблиц, Регистратор, ИсключатьВПериодКомандировок)
	...
	Запрос.Текст = 
		...
		|	РаспределяемыеСуммыПредварительно.СтратегияОтраженияВУчете КАК СтратегияОтраженияВУчете,
		|	ВЫБОР
		|		КОГДА РаспределяемыеСуммыПредварительно.СтратегияОтраженияВУчете В (ЗНАЧЕНИЕ(Перечисление.СтратегииОтраженияВУчетеНачисленийУдержаний.ПоДаннымОСотрудникеИЕгоПлановыхНачислениях),
		|				ЗНАЧЕНИЕ(Перечисление.СтратегииОтраженияВУчетеНачисленийУдержаний.КакЗаданоВидуРасчета)) ТОГДА
		|			РаспределяемыеСуммыПредварительно.СтатьяФинансирования
		...
		|КАК РаспределяемыеСуммыПредварительно
		|СГРУППИРОВАТЬ ПО
	...
	Запрос.Текст = 
		...
		|	БазаРаспределения.Индексируется КАК Индексируется,
		|	ВЫБОР
		|		КОГДА РаспределяемыеСуммы.СтратегияОтраженияВУчете В (ЗНАЧЕНИЕ(Перечисление.СтратегииОтраженияВУчетеНачисленийУдержаний.ПоДаннымОСотрудникеИЕгоПлановыхНачислениях),
		|				ЗНАЧЕНИЕ(Перечисление.СтратегииОтраженияВУчетеНачисленийУдержаний.КакЗаданоВидуРасчета)) ТОГДА
		...
		|				ИНАЧЕ ИСТИНА
		|			КОНЕЦ)
		|			И (РаспределяемыеСуммы.СтратегияОтраженияВУчете В (ЗНАЧЕНИЕ(Перечисление.СтратегииОтраженияВУчетеНачисленийУдержаний.ПоДаннымОСотрудникеИЕгоПлановыхНачислениях),
		|				ЗНАЧЕНИЕ(Перечисление.СтратегииОтраженияВУчетеНачисленийУдержаний.КакЗаданоВидуРасчета))
