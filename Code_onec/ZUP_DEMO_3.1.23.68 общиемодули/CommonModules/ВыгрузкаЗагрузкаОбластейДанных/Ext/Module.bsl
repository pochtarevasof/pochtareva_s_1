...
Функция ЗагрузитьДифференциальнуюКопию(
	...
	РезультатЗагрузки2 = ЗагрузитьТекущуюОбласть(
		...
		ДанныеРасширений, 
		"Дифференциальная", 
		РезультатЗагрузки1.СоответствиеСсылок);
	...
	РезультатЗагрузки2.Удалить("СоответствиеСсылок");
...
Функция ЗагрузитьТекущуюОбласть(
	...
	ПараметрыЗагрузки.Вставить("ТипРезервнойКопии", ТипРезервнойКопии);
	Если СоответствиеСсылок <> Неопределено Тогда
		ПараметрыЗагрузки.Вставить("СоответствиеСсылок", СоответствиеСсылок);
	Иначе
		ПараметрыЗагрузки.Вставить("ЭтоЗагрузкаПолногоБэкапа", );
	КонецЕсли;
