			...
			Если СтрокаРезультата.ФизическоеЛицо <> Неопределено И СтрокаРезультата.П000030001001 <> "1" Тогда
				...
				Если НоваяСтрока.ДанныеМногострочныхЧастей.П00321М1.Строки.Количество() = 0 Тогда
					...
					Если ЗначениеЗаполнено(СтрокаРезультата.НачалоРаботы) Тогда
						...
						КонецЕсли;
						СтрокаДохода.Данные.П00321М113001 = Перечисления.КатегорииЗастрахованныхЛицДляПФР.КодКатегорииДляОтчета(КатегорияНулевыхДоходов);
					КонецЕсли;
				КонецЕсли;
...
Процедура РасчетПоказателей_РСВ_2022(НачалоПериода, ОкончаниеПериода, Организация, ОписаниеТаблиц, ПодробныеДанные, ДатаАктуальности, МассивФизическихЛиц)
	...
	Если ОписаниеТаблиц.Свойство("ЧисленностьЗастрахованныхВРазрезеТарифов2022") Тогда 
		...
		Иначе
			...
			|ГДЕ
			|	Доходы.ЗастрахованФОМС
			|	И ЕСТЬNULL(ВидыТарифовИзУчетнойПолитики.ВидТарифаСтраховыхВзносов, ЗНАЧЕНИЕ(Справочник.ВидыТарифовСтраховыхВзносов.ОбщийНалоговыйРежим)) В (ЗНАЧЕНИЕ(Справочник.ВидыТарифовСтраховыхВзносов.ОрганизацииЗанимающиесяОбщепитом), ЗНАЧЕНИЕ(Справочник.ВидыТарифовСтраховыхВзносов.ДляСубъектовМалогоПредпринимательства))
			|	И Доходы.ВидТарифаСтраховыхВзносов В (ЗНАЧЕНИЕ(Справочник.ВидыТарифовСтраховыхВзносов.ОрганизацииЗанимающиесяОбщепитом), ЗНАЧЕНИЕ(Справочник.ВидыТарифовСтраховыхВзносов.ДляСубъектовМалогоПредпринимательства), ЗНАЧЕНИЕ(Справочник.ВидыТарифовСтраховыхВзносов.ОбщийНалоговыйРежим))
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
	...
	Если ОписаниеТаблиц.Свойство("Подраздел31Доходы2022") Тогда
		...
		"ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА СведенияОДоходах.ВидТарифаСтраховыхВзносов В (ЗНАЧЕНИЕ(Справочник.ВидыТарифовСтраховыхВзносов.ОрганизацииЗанимающиесяОбщепитом), ЗНАЧЕНИЕ(Справочник.ВидыТарифовСтраховыхВзносов.ДляСубъектовМалогоПредпринимательства))
	...
	Если ОписаниеТаблиц.Свойство("Подраздел31Численность2022") Тогда
		...
		|	ВЫБОР
		|		КОГДА Взносы.ВидТарифаСтраховыхВзносов В (ЗНАЧЕНИЕ(Справочник.ВидыТарифовСтраховыхВзносов.ОрганизацииЗанимающиесяОбщепитом), ЗНАЧЕНИЕ(Справочник.ВидыТарифовСтраховыхВзносов.ДляСубъектовМалогоПредпринимательства))
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыТарифовСтраховыхВзносов.ОбщийНалоговыйРежим)
	...
	Если ОписаниеТаблиц.Свойство("Подраздел31Взносы2022") Тогда
		...
		"ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА ВзносыФизлиц.ВидТарифаСтраховыхВзносов В (ЗНАЧЕНИЕ(Справочник.ВидыТарифовСтраховыхВзносов.ОрганизацииЗанимающиесяОбщепитом), ЗНАЧЕНИЕ(Справочник.ВидыТарифовСтраховыхВзносов.ДляСубъектовМалогоПредпринимательства))
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыТарифовСтраховыхВзносов.ОбщийНалоговыйРежим)
...
Процедура ОписаниеПоказателей_4ФСС_2017Кв1(ПоказателиОтчета, ПараметрыОтчета = Неопределено) Экспорт
	...
	УчетСтраховыхВзносовАрхивныеАлгоритмы.ОписаниеПоказателей_4ФСС_2017Кв1(ПоказателиОтчета, ПараметрыОтчета)
	
КонецПроцедуры

Процедура ЗначенияПоказателей_4ФСС_2017Кв1(ПараметрыОтчета, Контейнер) Экспорт
	
	УчетСтраховыхВзносовАрхивныеАлгоритмы.ЗначенияПоказателей_4ФСС_2017Кв1(ПараметрыОтчета, Контейнер)
