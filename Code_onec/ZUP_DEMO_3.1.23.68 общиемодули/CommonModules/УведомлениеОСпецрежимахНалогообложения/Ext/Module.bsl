...
Процедура ПроверкаКодаОКТМО(Обязательность, ОКТМО, ТаблицаОшибок, НаименованиеТитульнойСтраницы, ИмяПоля, УИД = Неопределено) Экспорт 
	Если Обязательность Тогда 
		Если Не ЗначениеЗаполнено(ОКТМО)
			Или (СтрДлина(ОКТМО) <> 8 И СтрДлина(ОКТМО) <> 11)
			Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ОКТМО) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Не указан / неправильно указан ОКТМО", НаименованиеТитульнойСтраницы, ИмяПоля, УИД));
		КонецЕсли;
	ИначеЕсли ЗначениеЗаполнено(ОКТМО) Тогда 
		Если (СтрДлина(ОКТМО) <> 8 И СтрДлина(ОКТМО) <> 11)
			Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ОКТМО) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам(
				"Неправильно указан ОКТМО", НаименованиеТитульнойСтраницы, ИмяПоля, УИД));
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
