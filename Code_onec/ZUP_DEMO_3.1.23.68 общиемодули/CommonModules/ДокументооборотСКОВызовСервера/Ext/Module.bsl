...
Функция ЗагрузитьМЧДФСС(ВходящиеДанные, ОбновлятьСуществующий = Ложь) Экспорт
...
Функция ПараметрыОтзываМашиночитаемойДоверенностиФСС(
	...
	ЗаписьXML.ЗаписатьТекст(?(ОтозватьЦепочку, "true", "false"));
	ЗаписьXML.ЗаписатьКонецЭлемента();
...
Функция НужноПоказатьРекламуТарифаКадровыеРешения(Организация = Неопределено) Экспорт
	...
	Скрыть = ЗагрузитьНастройку("КадровыеРешенияНапоминать") = Ложь;
...
Функция ЭтоТарифКадровыеРешения(Организация) Экспорт
	...
	Если ИспользуетсяРежимТестирования() Тогда
		Возврат Истина;
	Иначе
		Возврат Врег(УчетнаяЗапись.ЛицензияНаименование) = Врег(НСтр("ru = 'Кадровые решения'"));
	КонецЕсли;
