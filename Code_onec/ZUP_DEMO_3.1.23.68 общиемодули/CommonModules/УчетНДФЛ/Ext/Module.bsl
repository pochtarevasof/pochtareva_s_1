...
Функция НалогСДивидендовАкционера(ДатаПолученияДохода, Организация, Акционер, Начислено, ВычетПоНДФЛ, КатегорияДивидендов = Неопределено, ИсключаемыйРегистратор = Неопределено) Экспорт
	...
	Если ВыполнятьРасчетПоПрогрессивнойШкале Тогда
		...
		|	И СведенияОДоходахНДФЛ.КатегорияДохода В(&КатегорияДохода)
		...
		|	И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.КатегорияДохода В(&КатегорияДохода)
		...
		Запрос.УстановитьПараметр("КатегорияДохода", ?(ДатаПолученияДохода < ПараметрыРасчетаНДФЛПоПрогрессивнойШкале.ДатаОбъединенияНалоговыхБаз, КатегорияДивидендов, Перечисления.КатегорииДоходовНДФЛ.ПоОсновнойСтавке()));
...
Процедура СформироватьДоходыИНДФЛСДивидендов(Регистратор, Движения, Отказ, Организация, ДатаВыплаты, Дивиденды, ВключатьВДекларациюПоНалогуНаПрибыль, РегистрироватьУдержаниеПеречислениеНДФЛ = Ложь, ДатаПлатежа = Неопределено, РеквизитыПлатежа = "", Записывать = Ложь) Экспорт
	...
	Если ТипЗнч(Движения) <> Тип("Структура") И Движения.Найти("ДокументыУчтенныеПриРасчетеНДФЛ") <> Неопределено Тогда
		СформироватьДокументыУчтенныеПриРасчетеДляМежрасчетногоДокумента(Движения, Отказ, Организация, Дивиденды.ВыгрузитьКолонку("ФизическоеЛицо"), Регистратор, Записывать);
	КонецЕсли;
...
// Формирует таблицу сумм удержанного налога в КБК, сроков перечисления и регистраций в ИФНС.
//
// Параметры:
//		Организация - СправочникСсылка.Организации - должно быть непустым значением.
//		НачалоПериода - дата - 
...
Функция УдержаноНалогаЗаПериод(Организация, НачалоПериода, ОкончаниеПериода, ПолучатьДанныеПоВсемуЮрлицу = Ложь, ДляЕНП = Истина) Экспорт
	...
	|
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.КатегорииДоходовНДФЛ.ОплатаТрудаАУСН),
	|	&ОсновнойКБК
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.КатегорииДоходовНДФЛ.ПрочиеДоходыАУСН),
	|	&ОсновнойКБК
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	КОНЕЦПЕРИОДА(РасчетыНалогоплательщиковСБюджетомПоНДФЛ.Период, КВАРТАЛ) КАК КонецКварталаДляОтчета6НДФЛ
	|ПОМЕСТИТЬ ВТНалоги
	|ИЗ
	|	РегистрНакопления.РасчетыНалогоплательщиковСБюджетомПоНДФЛ КАК РасчетыНалогоплательщиковСБюджетомПоНДФЛ
	...
	Если ДляЕНП Тогда
		...
		ПроставитьКрайнийСрокУплаты(ВременнаяТаблица, , Ложь);
		
		// Исключим удержанные суммы, срок уплаты которых относится к следующему кварталу.
		СтрокиКУдалению = Новый Массив;
		Для каждого СтрокаТаблицы Из ВременнаяТаблица Цикл
			Если СтрокаТаблицы.Период <= СтрокаТаблицы.КонецКварталаДляОтчета6НДФЛ И СтрокаТаблицы.КрайнийСрокУплаты > СтрокаТаблицы.КонецКварталаДляОтчета6НДФЛ  Тогда
				СтрокиКУдалению.Добавить(СтрокаТаблицы)
			КонецЕсли;
		КонецЦикла;
		Для каждого УдаляемаяСтрока Из СтрокиКУдалению Цикл
			ВременнаяТаблица.Удалить(УдаляемаяСтрока);
		КонецЦикла;	
	...
	Результаты = Запрос.ВыполнитьПакет();
	Если ДляЕНП Тогда
		ВсеУдержания = Результаты[1].Выгрузить();
		Возвраты = Результаты[2];
	Иначе
		ВсеУдержания = Результаты[2].Выгрузить();
		Возвраты = Результаты[3];
	КонецЕсли;
	Если Не Возвраты.Пустой() Тогда
		
		ВсеВозвраты = Возвраты.Выгрузить();
		
		Измерения = "ФизическоеЛицо,КБК,Организация,РегистрацияВНалоговомОргане";
		...
		Для каждого СтрокаУдержания Из ВсеУдержания Цикл
			Если ТекущееФизическоеЛицо <> СтрокаУдержания.ФизическоеЛицо
				Или ТекущийКБК <> СтрокаУдержания.КБК
				...
				ТекущееФизическоеЛицо = СтрокаУдержания.ФизическоеЛицо;
			...
			Для каждого СтрокаВозврата Из СтрокиВозвратов Цикл
				...
				Если СтрокаУдержания.СрокПеречисленияНалога > СтрокаВозврата.ДатаВозврата Или СтрокаУдержания.Сумма = 0 Тогда
					Прервать;
				КонецЕсли;
	...
	ВсеУдержания.Свернуть("КБК,Организация,РегистрацияВНалоговомОргане,СрокПеречисленияНалога", "Сумма");
...
Процедура СформироватьДоходыНДФЛПоНачислениям(Движения, Отказ, Организация, ДатаОперации, ДатаВыплаты, МенеджерВременныхТаблиц, МесяцНачисления = Неопределено, Записывать = Ложь, ОкончательныйРасчет = Истина, ИмяВТНачисления = "ВТНачисления", Регистратор = Неопределено, МенеджерКадровогоУчета = Неопределено, ДоходПолученНаТерриторииРФ = Ложь) Экспорт
	...
	|			ТОГДА &ДатаВыплаты
	|		КОГДА ВидыРасчета.КатегорияДохода = ЗНАЧЕНИЕ(Перечисление.КатегорииДоходовНДФЛ.ОплатаТрудаАУСН)
	|			ТОГДА &ДатаВыплаты
	|		КОГДА ВидыРасчета.КатегорияДохода = ЗНАЧЕНИЕ(Перечисление.КатегорииДоходовНДФЛ.ПрочиеДоходыАУСН)
	|			ТОГДА &ДатаВыплаты
...
Процедура ЗарегистрироватьДоходыИСуммыНДФЛПриНачисленииАванса(Регистратор, Движения, Отказ, Организация, Дата, ПериодРегистрации, ПланируемаяДатаВыплаты, ДанныеДляПроведения, Записывать = Ложь, ИмяТаблицыФизлиц = "ВТФизическиеЛица") Экспорт 
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы") Тогда
		МодульОтражениеЗарплатыВУчетеРасширенный = ОбщегоНазначения.ОбщийМодуль("ОтражениеЗарплатыВУчетеРасширенный");
		МодульОтражениеЗарплатыВУчетеРасширенный.ДополнитьСведенияОДоходахНДФЛСведениямиОРаспределенииПоСтатьям(Движения, Истина);
	КонецЕсли;
	
	// Регистрация исчисленного налога.
	СформироватьНалогиВычеты(Движения, Отказ, Организация, ДатаОперацииПоНалогам, ДанныеДляПроведения.НДФЛ, , Ложь, ПланируемаяДатаВыплаты);
	...
	СформироватьДокументыУчтенныеПриРасчетеДляМежрасчетногоДокументаПоВременнойТаблице(Движения, Отказ, Организация, ДанныеДляПроведения.МенеджерВременныхТаблиц, Регистратор, ИмяТаблицыФизлиц); 	
...
Процедура РассчитатьНалогДляКонкретногоДохода(ДатаПолученияДохода, Доходы) Экспорт
	...
	|	ВЫБОР
	|		КОГДА &УсловиеОблагаемостиУНалоговогоАгента
	|			ТОГДА ВЫРАЗИТЬ((СписокДоходов.СуммаДохода - ВЫБОР
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеОблагаемостиУНалоговогоАгента", УсловиеОблагаемостиУНалоговогоАгента("СписокДоходов", "КодДохода", "Период"));
...
Процедура ВписатьСуммыВыплаченногоДоходаВУдержанныеНалоги(МенеджерВременныхТаблиц, Регистратор, ДатаВыплаты = Неопределено, ДатаОперации = Неопределено) Экспорт
	...
	|					ТОГДА СведенияОДоходахНДФЛ.Регистратор
	|				ИНАЧЕ СведенияОДоходахНДФЛ.ДокументОснование
	|			КОНЕЦ = ФизическиеЛица.ДокументОснование)
	|ГДЕ
...
//
Процедура ПроставитьКрайнийСрокУплаты(КоллекцияСтрокДвижений, Организация = Неопределено, ПереноситьНаРабочийДень = Истина) Экспорт
	
	Если ПереноситьНаРабочийДень Тогда
	...
	Для Каждого СтрокаНабора Из КоллекцияСтрокДвижений Цикл
		Если СтрокаНабора.ВариантУдержания = Перечисления.ВариантыУдержанияНДФЛ.Удержано И Не ЗначениеЗаполнено(СтрокаНабора.КрайнийСрокУплаты) Тогда	
			...
			КонецЕсли;
			Если ПереноситьНаРабочийДень И Не УчитыватьОсобыеНерабочиеДни Тогда
...
Функция УсловиеОблагаемостиУНалоговогоАгента(ИмяТаблицы = "СведенияОДоходахНДФЛ", ИмяПоляКодДохода = "КодДохода", ИмяПоляДатаПолученияДохода = "ДатаПолученияДохода") Экспорт

	ТекстУсловия = 
	"НЕ(СведенияОДоходахНДФЛ.КодДохода.НеОблагаетсяУНалоговогоАгента
	|	ИЛИ СведенияОДоходахНДФЛ.КодДохода.КодПрименяемыйВНалоговойОтчетностиС2021Года В (""2610"", ""2630"", ""2640"", ""2641"")
	|		И (СведенияОДоходахНДФЛ.ДатаПолученияДохода МЕЖДУ ДАТАВРЕМЯ(2021, 1, 1, 0, 0, 0) И ДАТАВРЕМЯ(2023, 12, 31, 23, 59, 59)))";
	
	Если ИмяТаблицы <> "СведенияОДоходахНДФЛ" Тогда
		Если ЗначениеЗаполнено(ИмяТаблицы) Тогда
			ТекстУсловия = СтрЗаменить(ТекстУсловия, "СведенияОДоходахНДФЛ", ИмяТаблицы)
		Иначе
			ТекстУсловия = СтрЗаменить(ТекстУсловия, "СведенияОДоходахНДФЛ.", "")
		КонецЕсли;
	КонецЕсли;
	Если ИмяПоляКодДохода <> "КодДохода" Тогда
		ТекстУсловия = СтрЗаменить(ТекстУсловия, "КодДохода", ИмяПоляКодДохода)
	КонецЕсли;
	Если ИмяПоляДатаПолученияДохода <> "ДатаПолученияДохода" Тогда
		ТекстУсловия = СтрЗаменить(ТекстУсловия, "ДатаПолученияДохода", ИмяПоляДатаПолученияДохода)
	КонецЕсли;
...
Функция УсловиеНеОблагаемостиУНалоговогоАгента(ИмяТаблицы = "СведенияОДоходахНДФЛ", ИмяПоляКодДохода = "КодДохода", ИмяПоляДатаПолученияДохода = "ДатаПолученияДохода") Экспорт

	ТекстУсловия = 
	"СведенияОДоходахНДФЛ.КодДохода.НеОблагаетсяУНалоговогоАгента
	|	ИЛИ СведенияОДоходахНДФЛ.КодДохода.КодПрименяемыйВНалоговойОтчетностиС2021Года В (""2610"", ""2630"", ""2640"", ""2641"")
	|		И (СведенияОДоходахНДФЛ.ДатаПолученияДохода МЕЖДУ ДАТАВРЕМЯ(2021, 1, 1, 0, 0, 0) И ДАТАВРЕМЯ(2023, 12, 31, 23, 59, 59))";
...
Функция ДоходОблагаетсяУНалоговогоАгента(КодДохода, ДатаПолученияДохода) Экспорт

	Возврат	НЕ (КодДохода.НеОблагаетсяУНалоговогоАгента
				Или (КодДохода.КодПрименяемыйВНалоговойОтчетностиС2021Года = "2610" Или КодДохода.КодПрименяемыйВНалоговойОтчетностиС2021Года = "2630"
						Или КодДохода.КодПрименяемыйВНалоговойОтчетностиС2021Года ="2640" Или КодДохода.КодПрименяемыйВНалоговойОтчетностиС2021Года = "2641")
					И ДатаПолученияДохода >= '20210101' 
					И ДатаПолученияДохода < '20240101')
...
Функция ПараметрыРасчетаНДФЛПоПрогрессивнойШкале() Экспорт 
	...
	Параметры.Вставить("ДатаОбъединенияНалоговыхБаз", '20230101');
	Параметры.Вставить("ГодОбъединенияНалоговыхБаз", 2023);
...
Процедура СоздатьВТСведенияОДоходахДляРаспределенияНДФЛ(ПараметрыПолученияБазы, ИсключаемыеРегистраторы) Экспорт
	...
	Если ОкончательныйРасчет Тогда
		...
		|		СведенияОДоходах.ДокументОснование КАК ДокументОснование
		|	ИЗ
		|		РегистрНакопления.СведенияОДоходахНДФЛ КАК СведенияОДоходах
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
...
Функция СправкиНДФЛДанныеДляПечати(МассивОбъектов, ПолучатьИтоговыеСуммы = Ложь) Экспорт
	...
	Если ПолучатьИтоговыеСуммы Тогда  // Требуется только для печати Реестра справок.
		...
		|	ДанныеДокумента.ОснованиеПодписи КАК ДокументПредставителя,
	...
	Иначе
	   ...
	   |	ДанныеДокумента.СправкуПодписал КАК СправкуПодписал,
	   |	ДанныеДокумента.ОснованиеПодписи КАК ДокументПредставителя,
	   |	&ОснованиеДляПредставленияСправок КАК ОснованиеДляПредставленияСправок,
	   |	Организации.ИНН КАК ИНН,
	   |	Организации.ЮридическоеФизическоеЛицо КАК ЮридическоеФизическоеЛицо,
	   |	Организации.НаименованиеПолное КАК НаименованиеПолное,
	   |	Организации.НаименованиеСокращенное КАК НаименованиеСокращенное,
	...
	Если ЭтоСправкаДляНалоговогоОргана Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ДанныеДокумента.ОснованиеПодписи", """"""); 
	...
	Иначе	
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОснованиеДляПредставленияСправок КАК ОснованиеДляПредставленияСправок", "ДанныеДокумента.СпособФормирования КАК СпособФормирования");
	КонецЕсли;			   
	...
	|	ДанныеДокументов.ДокументПредставителя КАК ДокументПредставителя,
	...
	Пока ДанныеОрганизации.Следующий() Цикл 
        ...
        ДанныеНалоговогоАгента.Вставить("ДокументПредставителя", ДанныеОрганизации.ДокументПредставителя);
        ...
        ДанныеПредставителя = Новый Структура("ТипПодписанта, ПредставительЮрЛицо, НаименованиеОрганизацииПредставителя, Фамилия, Имя, Отчество, ФИОПредставителя", "1", Истина, "", "", "", "", "", "");
...
Функция ОписаниеДоходовОрганизации(Организация, НалоговыйПериод, ДатаПодачиСведений, ФизическоеЛицо = Неопределено, ДляНалогаНаПрибыль = Ложь, НеВключатьДоходыПоЦеннымБумагам = Ложь) Экспорт
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеОблагаемостиУНалоговогоАгента", УсловиеОблагаемостиУНалоговогоАгента("НДФЛСведенияОДоходах"));
...
Функция ДанныеОПерерасчетеНДФЛ(Организация, МесяцПерерасчета, НачалоПериода, ОкончаниеПериода, ФизическиеЛица = Неопределено, ДокументСсылка = Неопределено) Экспорт
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеОблагаемостиУНалоговогоАгента", УсловиеОблагаемостиУНалоговогоАгента("Обороты"));
	Если ЗначениеЗаполнено(ФизическиеЛица) Тогда
		Запрос.УстановитьПараметр("ФизическиеЛица", ФизическиеЛица);
	...
	Если ОкончаниеПериода > ПараметрыРасчетаНДФЛПоПрогрессивнойШкале.ДатаОбъединенияНалоговыхБаз Тогда
		Для каждого СтрокаРезультата Из РезультатРасчетаНДФЛ Цикл
			Если СтрокаРезультата.КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.Дивиденды Тогда
				СтрокаРезультата.НалогПоСтавке09 = СтрокаРезультата.НалогПоСтавке13;
				СтрокаРезультата.НалогПоСтавке13 = 0;
			КонецЕсли;	
		КонецЦикла;
	КонецЕсли;
	...
	|ГДЕ
	|	Доходы.СтавкаНалогообложенияРезидента <> ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка13)
	|	И Доходы.ОблагаетсяУНалоговогоАгента
	...
	|	И Доходы.ДатаПолученияДохода < &ДатаОбъединенияНалоговыхБаз
	|	И Доходы.ОблагаетсяУНалоговогоАгента
	|
	|СГРУППИРОВАТЬ ПО
	|	Доходы.ФизическоеЛицо,
	...
	|ГДЕ
	|	(РезидентыСДивидендами.ФизическоеЛицо ЕСТЬ NULL
	|			ИЛИ РанееНачисленныеНалоги.МесяцНалоговогоПериода < &ДатаОбъединенияНалоговыхБаз)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция УдерживатьНалогПриВыплатеАванса(Организация) Экспорт
	...
	Возврат УчетНДФЛВнутренний.УдерживатьНалогПриВыплатеАванса(Организация);
...
Функция ЗапросПраваНаСтандартныеВычетыПоНДФЛДляАналитическогоОтчета(ТолькоРазрешенные, НачалоПериода, ОкончаниеПериода, ДатаАктуальности, ИмяВременнойТаблицыОтбора = "ВТФизическиеЛица")
	...
	|					ТОГДА ЛОЖЬ
	|				КОГДА &УсловиеНеОблагаемостиУНалоговогоАгента
	|					ТОГДА ЛОЖЬ
	|				КОГДА СтатусыФизлиц.Статус = ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.Резидент)
	|					ТОГДА ИСТИНА
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеНеОблагаемостиУНалоговогоАгента", УсловиеНеОблагаемостиУНалоговогоАгента("Доходы"));
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.Справочники.ВидыДоходовНДФЛ) Тогда
		...
		Обработчик.Процедура = "Справочники.ВидыДоходовНДФЛ.ОписатьДоход2015";
		Обработчик.ОбщиеДанные = Истина;
