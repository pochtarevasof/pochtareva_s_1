...
//   ПараметрыОбновления - Структура - Параметры отложенного обновления.
//
Процедура ПовторноОбработатьВходящиеСообщенияСЭДО(ПараметрыОбновления = Неопределено, ТипыЧислом = Неопределено) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВходящиеСообщенияСЭДОФСС.Тип В (&Типы)", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И (ВходящиеСообщенияСЭДОФСС.Дата >= &Дата", "(ВходящиеСообщенияСЭДОФСС.Дата >= &Дата");
	КонецЕсли;
	Запрос.УстановитьПараметр("Дата", '20220101');
...
Процедура ПовторноОбработатьРегистрацииОтветовНаЗапросыФССДляРасчетаПособий(ПараметрыОбновления = Неопределено) Экспорт
	ТипыЧислом = Новый Массив;
	ТипыЧислом.Добавить(105);
	ПовторноОбработатьВходящиеСообщенияСЭДО(Неопределено, ТипыЧислом);
	Если ПараметрыОбновления <> Неопределено Тогда
		ПараметрыОбновления.ОбработкаЗавершена = Истина;
	КонецЕсли;
КонецПроцедуры
...
//
Процедура ОшибкаОбработки(Результат, ИдентификаторСообщения, ТекстОшибки, ТекстXML = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ТекстXML) Тогда
		Результат.ОписаниеОшибки = Результат.ОписаниеОшибки
			+ Символы.ПС
			+ СтрШаблон(НСтр("ru = 'Текст XML: %1'"), ТекстXML);
	КонецЕсли;
...
Функция СтрахователиГоловнойОрганизации(ГоловнаяОрганизация) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ВестиУчетОтчитывающихсяПоВзносамОрганизаций") Тогда
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОтчитывающиесяПоВзносамОрганизации КАК ОтчитывающиесяПоВзносамОрганизации
		|		ПО ВТФилиалы.Ссылка = ОтчитывающиесяПоВзносамОрганизации.Организация";
	Иначе
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Организации.Ссылка КАК Ссылка
...
Процедура СвертываемыеГруппыАдаптироватьСообщениеПользователю(Форма, Сообщение) Экспорт
	...
	Если Элемент = Неопределено Или ТипЗнч(Элемент) <> Тип("ПолеФормы") Тогда
		Возврат;
	КонецЕсли;
...
Функция СтрокаИзBase64(СтрокаBase64)
	...
	Чтение.Закрыть();
	Если Не ЗначениеЗаполнено(Строка) И Не ПустаяСтрока(СтрокаBase64) Тогда
...
//   ВключитьБизнесЛогику - Булево, Неопределено - Включает бизнес-логику при записи объекта.
...
Процедура ЗаписатьДокумент(Знач ДокументОбъект, Знач ОтправлятьВДругиеУзлы = Неопределено, 
	...
	// Включение бизнес-логики БЗК в случае, если документ записывается при получении данных ФСС (не при обновлении ИБ).
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РегистрацияСведенийОЗастрахованномЛицеФСС")
		И ВключитьБизнесЛогику <> Истина
		И (ОтправлятьВДругиеУзлы <> Неопределено Или РежимЗаписи <> Неопределено) Тогда
	...
	Если ОтправлятьВДругиеУзлы = Неопределено Тогда
		...
		ОтправлятьВДругиеУзлы = ДокументОбъект.ЭтоНовый();
	...
	Иначе
		...
		Если Не ОтправлятьВДругиеУзлы И Не ДокументОбъект.ЭтоНовый() Тогда
...
Функция СписокВыбораСпособовВыплаты(СписокВыбора = Неопределено, ТекущееЗначение = Неопределено, КатегорияЗастрахованногоЛица = Неопределено) Экспорт
	...
	Если ТекущееЗначение = ИнаяОрганизация Или КатегорияЗастрахованногоЛица = "PRISONER" Тогда
