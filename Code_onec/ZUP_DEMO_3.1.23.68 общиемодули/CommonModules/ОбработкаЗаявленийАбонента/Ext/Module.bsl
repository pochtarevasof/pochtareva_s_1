...
Процедура ЗаполнитьМЧДВЗаявлении(Форма) Экспорт
	...
	ДобавитьМЧДФНСВЗаявление(Форма, Доверенности);
...
Функция ДобавитьМЧДФНСВЗаявление(Форма, Доверенности) Экспорт
	
	Выборка = ВыборкаМЧДДляЗаявления(Форма, "ФНС");
	
	Пока Выборка.Следующий() Цикл
		
		МЧД = Выборка.МЧД;
		
		Отправлена = МЧДФНСОтправлена(МЧД);
		Отозвана   = МЧДФНСОтозвана(МЧД);
		Загружена  = ОбработкаЗаявленийАбонентаВызовСервера.МЧДФНСЗагружена(МЧД);
...
Функция МЧДФНСОтправлена(МЧД) Экспорт
...
Функция МЧДФНСОтозвана(МЧД) Экспорт
...
Функция ВыборкаМЧДДляЗаявления(Форма, Орган) Экспорт
	...
	Запрос.Текст = 
		...
		|			ИЛИ ТИПЗНАЧЕНИЯ(МЧД.Представитель) <> ТИП(Справочник.ФизическиеЛица))";
	
 	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ФСС", Орган);
	
	Запрос.УстановитьПараметр("ТекущаяДата", 	ТекущаяДатаСеанса());
	Запрос.УстановитьПараметр("Доверитель", 	Форма.Организация);
	Запрос.УстановитьПараметр("Представитель", 	Форма.ВладелецЭЦП);
