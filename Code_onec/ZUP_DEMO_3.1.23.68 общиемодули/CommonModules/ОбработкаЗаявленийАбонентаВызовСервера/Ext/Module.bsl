...
Функция ПолучитьФайлыМЧД(МЧД, ВладелецЭЦПИНН) Экспорт
	...
	ЭтоМЧДФНС = ТипЗнч(МЧД) = Тип("СправочникСсылка.МашиночитаемыеДоверенностиФНС");
	
	Если ЭтоМЧДФНС Тогда
		Отправлена = ОбработкаЗаявленийАбонента.МЧДФНСОтправлена(МЧД);
		Загружена  = МЧДФНСЗагружена(МЧД);
		Если НЕ Отправлена И НЕ Загружена Тогда
			Результат.ТекстОшибки = ДополнитьОшибкуМЧД(МЧД, НСтр("ru = 'она не отправлена'"));
			Возврат Результат;
		КонецЕсли;
		
		Отозвана = ОбработкаЗаявленийАбонента.МЧДФНСОтозвана(МЧД);
		Если Отозвана Тогда
...
Функция ФайлыПодписиМЧД(Доверенность) Экспорт
	...
	МЧДФНСЗагружена = МЧДФНСЗагружена(Доверенность);
	Если МЧДФНСЗагружена Тогда
